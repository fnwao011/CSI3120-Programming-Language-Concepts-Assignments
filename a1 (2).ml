...
print_survey();;
(* Did not print in terminal unless i wrote this *)
...
let exp3a : string = " addition of 2 different types"
...
let exp3b1 : string = "I dont see anything wrong it already compiles so i put recusive call in parenthesis and removed 2 semi colons"
...
let exp4 : string = "It is not running anything and ther is no stop condition so it recureses forever. It compiles though."
...
let rec prob5(pal:string) : bool =
	if String.length pal < 1 then true
	else if String.get pal 0 != String.get pal(String.length pal - 1) then false
	else prob5 (String.sub pal 1 (String.length pal - 2));;
